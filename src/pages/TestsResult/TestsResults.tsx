...
import { Button, Card, Empty, Input, Menu, message, Modal, Progress, Result, Row, Space, theme, Typography } from "antd";
    ...
    return (
        <MainLayout>
            <div style={{ display: 'flex' }}>
                {
                    isError && !isLoading &&
                    <Card style={{ width: '300px', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                        <Empty
                            style={{ margin: '0 auto' }}
                ...
                {
                    activeElement ?
                        <Card title={test?.name} extra={test ? `${test.questions.length + 1} вопросов` : ''} loading={isTestsLoading} style={{ marginLeft: '50px', width: '100%' }}>
                                        ...
                                        resultChartData.map((item) => {
                                            return (
                                                <div key={item.id} style={{ display: 'flex', flexDirection: 'column' }}>
                                                    <Progress type="circle" percent={item.result} strokeWidth={10} />
                                                    <Text>{item.name}</Text>
                                                </div>
                                            )
                                        })
                            ...
                            }
                            {
                                ...
                                <div style={{ marginTop: '25px' }}>
                                    ...
                                    <Space style={{ marginTop: '50px' }}>
                                        <Link to={`/quiz/${activeElement}`}><Button type="primary">Открыть тест</Button></Link>
                                        <Button style={{ margin: '0 0 auto' }}>Список ответов</Button>
                                        <Button type="dashed" danger>Удалить тест</Button>
                                    </Space>
                                    <Modal title="Добавление критерия" open={isModalOpen} onOk={() => handleAddCriteria()} onCancel={() => { setIsModalOpen(false) }}>
                                        <Input
                                            style={{ margin: '10px' }} size="large" value={criteriaName}
